        ...
        public override void Initialize()
        {
            ...
            foreach (XmlNode animationDefinition in doc.GetElementsByTagName("Definition"))
            {
                ...
                if (toLoad == isAI || null == toLoad)
                {
                    ...
                    Texture2D texture =
                        curGame.Content.Load<Texture2D>(animationDefinition.Attributes["SheetName"].Value);

                    // Get the frame size (width & height)
                    Point frameSize = new Point();
                    frameSize.X = int.Parse(animationDefinition.Attributes["FrameWidth"].Value);
                    frameSize.Y = int.Parse(animationDefinition.Attributes["FrameHeight"].Value);

                    // Get the frames sheet dimensions
                    Point sheetSize = new Point();
                    sheetSize.X = int.Parse(animationDefinition.Attributes["SheetColumns"].Value);
                    sheetSize.Y = int.Parse(animationDefinition.Attributes["SheetRows"].Value);
                    ...
                    {
                        animation.Offset = new Vector2(int.Parse(animationDefinition.Attributes["OffsetX"].Value),
